...
import { Typography, Card, List, Button, Spin, Form, Input, message, Popconfirm, Modal, Avatar } from 'antd'; // Import Avatar
...
const PostDetail = () => {
  ...
  const getInitials = (username) => {
    if (!username) return 'U'; // 'U' for Unknown user
  ...
  return (
    <div className="container mx-auto p-6">
      {!isEditingPost ? (
        ...
        >
          {/* Display Post Author with Profile Picture */}
          <div className="flex items-center mb-2">
            {post.author?.profileImageUrl ? ( // Use optional chaining for safety
              <Avatar src={post.author.profileImageUrl} size="small" className="mr-2" />
            ) : (
              <Avatar className="mr-2">{getInitials(post.author?.username)}</Avatar>
            )}
      ...
      <List
        ...
        renderItem={item => (
          ...
          >
            <div className="flex items-center"> {/* Flex container for avatar and text */}
              {/* Display Comment Author with Profile Picture */}
              {item.author?.profileImageUrl ? ( // Use optional chaining for safety
                <Avatar src={item.author.profileImageUrl} size="small" className="mr-2" />
              ) : (
                <Avatar className="mr-2">{getInitials(item.author?.username)}</Avatar>
              )}
              <div>
