...
import './App.css';
import { message, Button, Spin, Avatar } from 'antd'; // Import Avatar component from antd
        ...
        {loadingAuth ? (
          ...
          <>
            <li className="ml-auto flex items-center space-x-3">
              {/* Display profile image if available, otherwise display initials */}
              {currentUser.profileImageUrl ? (
                <Avatar src={currentUser.profileImageUrl} size="default" />
              ) : (
...
function App() {
  ...
  const checkAuthStatus = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Assuming data.user includes profileImageUrl if available
  ...
  const handleLogin = useCallback((userData) => {
    // Assuming userData includes profileImageUrl if available
